...
const Reactors = ({data}: Props) => {
    ...
    // useMemo used because we don't want to re-calculate the random subarray on every render
    const randomSubarray = useMemo(() => {
        ...
        return getRandomSubarray(data || [], 8);
    }, [data]);
    ...
    return (
        <div className='px-5 hidden sm:block'>
            ...
            {randomSubarray.map((reactor: any, index: number) => (
