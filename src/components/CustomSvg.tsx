...
interface CustomSvgProps {
    name: 'world' | 'like'; // Add more SVG names here
    size?: string;
    color?: string;
}
...
const CustomSvg: FC<CustomSvgProps> = ({ name, size = '1em', color = 'currentColor' }) => {
    const SvgIcon = SVGs[name];
    if (!SvgIcon) {
        return null; // return null if the name prop doesn't match any SVG
    }
    const style: CSSProperties = { width: size, height: size, fill: color };
    return React.cloneElement(SvgIcon, { style });
};
